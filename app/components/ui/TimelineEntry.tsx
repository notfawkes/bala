  ...
  return (
    ...
    >
      {/* <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl text-foreground max-w-4xl">
          My journey
        </h2>
      </div> */}
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 font-bold text-foreground">
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        <div
          ...
          className="absolute left-8 top-0 w-0.5 bg-border/40
          mask-[linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]"
        >
          <motion.div
            style={{ height: heightTransform, opacity: opacityTransform }}
            className="absolute inset-x-0 top-0 w-0.5 bg-foreground rounded-full"
          />
        </div>
      </div>
    </div>
