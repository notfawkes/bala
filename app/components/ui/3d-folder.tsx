    ...
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-center mb-4" style={{ height: "160px", width: "200px" }}>
          ...
          <div
            className="absolute w-32 h-24 bg-foreground rounded-lg shadow-md"
            style={{
          ...
          <div
            className="absolute w-12 h-4 bg-foreground rounded-t-md"
            style={{
...
export function ImageLightbox({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-[#e8d6ad]"
        style={{
            ...
            >
              {projects.map((project, idx) => (
                <img
                  ...
                  alt={project.title}
                  className="w-full h-auto max-h-[70vh] object-contain bg-foreground flex-shrink-0"
                  style={{ minWidth: "100%" }}
                />
              ))}
            </div>
...
export const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ image, title, delay, isVisible, index, onClick, isSelected, projectId }, ref) => {
    ...
    return (
      <div
        ...
        onClick={(e) => {
          e.stopPropagation()
          // primary click navigates to project page if projectId provided
          const anchor = projectId
          if (anchor) {
            // use location so it works in all contexts and preserves SPA nav
            const url = `/projects/${anchor}`
            // allow open in new tab with ctrl/cmd/shift/alt
            const newTab = e.metaKey || e.ctrlKey || e.shiftKey || e.altKey
            if (newTab) {
